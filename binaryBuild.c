...
void writeBinaryParts(FILE *obFile,gNode row,gNode labels){
    ...
    if (getType(row)==JUMP) {
        ...
        if (getOpType(row,2)==IMMEDIATE) {op2 = getOp(row,2)<<2;}
        if (getOpType(row,3)==IMMEDIATE) {op3 = getOp(row,3)<<2;}

        if (getOpType(row,2)==DIRECT_REG) {op2 = getOp(row,2)<<8;}
        if (getOpType(row,3)==DIRECT_REG) {op3 = getOp(row,3)<<2;}
        ...
        if (getOpType(row,1)==IMMEDIATE) {op1 = getOp(row,1)<<2;}
        if (getOpType(row,2)==IMMEDIATE) {op2 = getOp(row,2)<<2;}

        if (getOpType(row,1)==DIRECT_REG) {op1 = getOp(row,1)<<8;}
        if (getOpType(row,2)==DIRECT_REG) {op2 = getOp(row,2)<<2;}
